...
public sealed class SqliteCruiseAlertSettingsRepository(KrytenAssistDbContext dbContext) : ICruiseAlertSettingsRepository
{
    ...
    public async Task<CruiseAlertSettings> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        return value is null ? new() : new(value.PriceDropEnabled, value.PromotionEnabled, value.SavedCriteriaEnabled, value.MinimumPriceDropPercentage);
    }
    ...
    public async Task SaveAsync(CruiseAlertSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        await dbContext.Database.ExecuteSqlInterpolatedAsync($"""
            ...
            VALUES ({ProfileId}, {settings.PriceDropEnabled}, {settings.PromotionEnabled}, {settings.SavedCriteriaEnabled}, {settings.MinimumPriceDropPercentage.ToString("G29", System.Globalization.CultureInfo.InvariantCulture)})
