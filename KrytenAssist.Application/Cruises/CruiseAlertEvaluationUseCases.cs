...
public sealed class EvaluateSavedCruiseCriteriaAlerts(SavedCruiseCriteriaAlertDetector detector, ICruiseAlertSettingsRepository settings, ISavedCruiseCriteriaStateRepository states, MaterializeCruiseAlertCandidates materialize)
{
    public async Task<CruiseAlertEvaluationResult> ExecuteAsync(SavedCruise saved, CruisePreferences preferences, CruiseCriteriaEvidence evidence, DateTimeOffset createdAt, CancellationToken token = default)
    {
        ...
        try { var currentSettings = await settings.GetAsync(token); var fingerprint = SavedCruiseCriteriaAlertDetector.CriteriaFingerprint(preferences, currentSettings); var prior = await states.GetAsync(saved.SailingKey, fingerprint, token); var detected = detector.Detect(saved, preferences, currentSettings, evidence, prior); var result = detected.Candidate is null ? CruiseAlertEvaluationResult.Success([], state: detected.State) : await materialize.ExecuteAsync([detected.Candidate], createdAt, token); await states.UpsertAsync(detected.State, token); return result with { CriteriaState = detected.State }; }
        catch (OperationCanceledException) { return CruiseAlertEvaluationResult.Cancelled(); } catch { return CruiseAlertEvaluationResult.Failed(); }
    }
}
